...
u = zeros(NF,1); f = zeros(NF,1); vr = zeros(NF,1);
qlast = 0;
...
u(n) = 2*k*(q+vB)+u(n-2); vr(n-1) = q;
end
%%%%%% end main loop
% plot mass displacement and relative bow velocity
tax = [0:NF-1]*k;
subplot(2,1,1); plot(tax, u, 'k');
title('Displacement of Mass'); xlabel('time (s)');
subplot(2,1,2); plot(tax, vr, 'k');
title('Relative Bow Velocity'); xlabel('time (s)');